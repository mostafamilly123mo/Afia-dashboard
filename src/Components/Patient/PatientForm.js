...
import { Container, Breadcrumb, Button, Row, Col, FormLabel, Alert, Spinner } from 'react-bootstrap'
import { connect } from 'react-redux';
import { Control, Form, Errors } from 'react-redux-form';
import { Link, withRouter } from 'react-router-dom';
import ModalPatient from '../../modals/ModalPatient';
import { actions } from 'react-redux-form'
import { checkPatientEmail, checkPatientUserName, closePatientRegisterDialog, openPatientDialog, resetPatientFileErrMess } from '../../redux/Actions/PatientAction';
...
function PatientForm(props) {
    ...
    const ErrorAlert = () => {
        if (props.patientRegisterStatus.isPatientAdded === false) {
            ...
            return (
                <Alert variant="success" className="addingAlert " onClose={() => {
                    props.closeRegisterDialog()
                    props.resetPatientFileErrMess()
                }} dismissible>
                    <Alert.Heading>
                        Success
                    </Alert.Heading>
                    <p>
                        ...
                        {
                            props.patientRegisterStatus.isFileLoading ? <span className="ps-1" className="text-primary">Patient File Loading <Spinner
                                as="span"
                                animation="grow"
                                size="sm"
                                role="status"
                        ...
                        {props.patientRegisterStatus.patientFileErrMess ? <p className="text-danger d-inline-block">{props.patientRegisterStatus.patientFileErrMess}</p> : <></>}
...
const mapDispatchToProps = (dispatch) => ({
    ...
    resetPatientFileErrMess: () => dispatch(resetPatientFileErrMess())
})
